# Python [ SpeedTest ] Por: @Carlos @Victor
...
def shell():
    ...
    printer('Testando de %(isp)s (%(ip)s)...' % speedtest.config['client'],
            quiet)

    if not args.mini:
        printer('recuperando lista de servidores speedtest.net ...', quiet)
        try:
    ...
    if args.download:
        printer('Testando velocidade de download\n', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.download(
            callback=callback,
        ...
        printer('Pular teste de download', quiet)
